...
def test_compute_query_desc(dataset, resnet18_gem):
    dataloader = dataset.query_images_loader(batch_size=1, preprocess=resnet18_gem.preprocess)
...
def test_compute_map_desc(dataset, resnet18_gem):
    dataloader = dataset.map_images_loader(batch_size=1, preprocess=resnet18_gem.preprocess)
    dataset = dataloader.dataset 
    dataset = Subset(dataset, list(range(2)))
    dataloader = DataLoader(dataset, batch_size=1)
