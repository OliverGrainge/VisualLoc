...
class TestMetrics(unittest.TestCase):
    def test_precision(self):
        ...
        prec = precision(ground_truth=ground_truth, 
                         preds=preds)
        ...
        prec = precision(ground_truth=ground_truth,
                         preds=preds,
                         ground_truth_soft=ground_truth)
    ...
    def test_recallatk(self):
        ...
        ratk = recallatk(ground_truth=ground_truth, 
                         similarity=similarity, k=1)
        assert ratk == 1.

        ratk = recallatk(ground_truth=ground_truth, 
                         similarity=similarity, k=3)
        assert ratk == 1.

        ratk = recallatk(ground_truth=ground_truth, 
                         similarity=similarity, k=5)
        assert ratk == 1.
