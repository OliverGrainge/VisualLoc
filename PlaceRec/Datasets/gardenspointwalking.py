...
class GardensPointWalking(BaseDataset):
    def __init__(self):
        ...
        if not os.path.isdir(package_directory + "/raw_images/GardensPointWalking"):
            ...
            s3_bucket_download(
                "placerecdata/datasets/GardensPointWalking.zip",
        ...
        self.map_paths = np.array(
            sorted(
                glob.glob(
                    package_directory + "/raw_images/GardensPointWalking/night_right/*"
                )
            )
        )
        self.query_paths = np.array(
            sorted(
                glob.glob(
                    package_directory + "/raw_images/GardensPointWalking/day_right/*"
                )
            )
        )
    ...
    def query_partition(self, partition: str) -> None:
    ...
    def map_partition(self, partition: str) -> None:
    ...
    def ground_truth(self, partition: str, gt_type: str) -> np.ndarray:
        ...
        if gt_type == "soft":
            gt = convolve2d(
                gt.astype(int), np.ones((17, 1), "int"), mode="same"
            ).astype("bool")
        elif gt_type == "hard":
            pass
        else:
