...
class InriaHolidays(BaseDataset):
    def __init__(self):
        ...
        self.map_paths = np.array([join(self.root, pth) for pth in np.load(join(self.root, "Inria_dbImages.npy"))])
        self.query_paths = np.array([join(self.root, pth) for pth in np.load(join(self.root, "Inria_qImages.npy"))])
