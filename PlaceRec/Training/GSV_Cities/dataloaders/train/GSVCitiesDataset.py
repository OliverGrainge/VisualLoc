...
class GSVCitiesDistillationDataset(Dataset):
    def __init__(
        self,
        cities=["London", "Boston"],
        img_per_place=4,
        ...
        super(GSVCitiesDistillationDataset, self).__init__()
        self.base_path = base_path
        self.cities = cities
        ...
        self.places_ids = pd.unique(self.dataframe.index)
        self.total_nb_images = len(self.dataframe)
        self.teacher_resize = T.Resize(teacher_img_size)
    ...
    def __getitem__(self, index):
        ...
        for i, row in place.iterrows():
            ...
            if self.transform is not None:
                img = self.transform(img)
            imgs_teacher.append(to_tensor(self.teacher_resize(to_pil_image(img))))
            imgs.append(img)
        ...
        # in the Dataloader (which will yield batches of shape [BS, K, channels, height, width])
        return (
            torch.stack(imgs_teacher),
            torch.stack(imgs),
            torch.tensor(place_id).repeat(self.img_per_place),
        )
