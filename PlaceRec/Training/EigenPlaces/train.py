...
output_folder = os.path.join(
    os.getcwd(),
    f"Checkpoints/eigenplaces/{args.save_dir}/{start_time.strftime('%Y-%m-%d_%H-%M-%S')}",
)
...
def train_eigenplaces(model, features_dim):
    ...
    classifiers = [
        cosface_loss.MarginCosineProduct(
            args.fc_output_dim, len(group), s=args.s, m=args.m
        )
        for group in groups
    ]
    classifiers_optimizers = [
        torch.optim.Adam(classifier.parameters(), lr=args.classifiers_lr)
    ...
    gpu_augmentation = tfm.Compose(
        [
            ...
            augmentations.DeviceAgnosticRandomResizedCrop(
                train_args.image_resolution, scale=[1 - args.random_resized_crop, 1]
            ),
    ...
    logging.info(
        f"The {len(groups)} groups have respectively the following "
    ...
    logging.info(
        f"There are {len(groups[0])} classes for the first group, "
    ...
    if args.resume_train:
        ...
        logging.info(
            f"Resuming from epoch {start_epoch_num} with best R@1 {best_val_recall1:.1f} "
    ...
    for epoch_num in range(start_epoch_num, args.epochs_num):
        ...
        def get_iterator(
            groups, classifiers, classifiers_optimizers, batch_size, g_num
        ):
            ...
            return commons.InfiniteDataLoader(
                groups[g_num],
        ...
        for i in range(2):
            classifiers[current_dataset_num + i] = classifiers[
                current_dataset_num + i
            ].cpu()
            util.move_to_device(classifiers_optimizers[current_dataset_num + i], "cpu")

        logging.debug(
            f"Epoch {epoch_num:02d} in {str(datetime.now() - epoch_start_time)[:-7]} - "
        ...
        logging.info(
            f"Epoch {epoch_num:02d} in {str(datetime.now() - epoch_start_time)[:-7]}, {val_ds}: {recalls_str}"
        )
        ...
        util.save_checkpoint(
            {
                ...
                "optimizers_state_dict": [
                    c.state_dict() for c in classifiers_optimizers
                ],
