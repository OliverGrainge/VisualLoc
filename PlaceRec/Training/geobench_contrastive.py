...
class GeoBaseDataset(data.Dataset):
    ...
    def _test_query_transform(self, img):
        ...
        if self.test_method == "single_query":
            ...
            processed_img = T.functional.resize(img, shorter_side, antialias=True)
...
class TripletsDataset(GeoBaseDataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train", negs_num_per_query=10):
        ...
        self.resized_transform = T.Compose([T.Resize(self.resize) if self.resize is not None else identity_transform, base_transform])
        ...
        self.hard_positives_per_query = list(
            knn.radius_neighbors(self.queries_utms, radius=args.train_positives_dist_threshold, return_distance=False)  # 10 meters
        )
        ...
        if self.mining == "msls_weighted":
            ...
            logging.info(
                f"#sideways_indexes [{len(sideways_indexes)}/{self.queries_num}]; " + "#night_indexes; [{len(night_indexes)}/{self.queries_num}]"
            )
    ...
    def compute_cache(args, model, subset_ds, cache_shape):
        ...
        subset_dl = DataLoader(
            dataset=subset_ds, num_workers=args.num_workers, batch_size=args.infer_batch_size, shuffle=False, pin_memory=(args.device == "cuda")
        )
        model = model.eval()
