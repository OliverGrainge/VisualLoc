...
def pr_curve(
    ...
    if matching == "single":
        ...
        ground_truth = ground_truth[
            np.argmax(similarity, axis=0), np.arange(ground_truth.shape[1])
        ]
...
def plot_selection_histogram(
    query_desc: dict, methods: list, dataset_name: str, show: bool = False
) -> None:
    selections = query_desc["selections"]
    method_names = [method.name for method in methods]
    fig, ax = plt.subplots()
    bins = np.arange(selections.min() - 0.5, selections.max() + 1.5, 1)
    ax.hist(selections, bins=bins)
    ax.set_xticks(np.arange(len(methods)))
    ax.set_xticklabels(method_names)
    ax.set_title("MultiPlex VPR Technique Selections")
