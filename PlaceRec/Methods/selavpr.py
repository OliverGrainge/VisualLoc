...
class Adapter(
    nn.Module
):  # Adapter is used to add to the transformer block for global adaptation
    def __init__(
        self, D_features, mlp_ratio=0.75, act_layer=nn.ReLU, skip_connect=True
    ):
...
class Block(nn.Module):
    def __init__(
        ...
        self.adapter2 = Adapter(
            dim, mlp_ratio=0.5, skip_connect=False
        )  # Parallel adapter
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        def ffn_residual_func(x: Tensor) -> Tensor:
            return self.ls2(
                self.mlp(self.norm2(x)) + 0.2 * self.adapter2(self.norm2(x))
            )  # 0.2 is the scaling factor for Parallel adapter
...
class NestedTensorBlock(Block):
    def forward_nested(self, x_list: List[Tensor]) -> List[Tensor]:
        ...
        if self.training and self.sample_drop_ratio > 0.0:
            ...
            def attn_residual_func(x: Tensor, attn_bias=None) -> Tensor:
                return self.ls1(
                    self.adapter1(self.attn(self.norm1(x), attn_bias=attn_bias))
                )
        ...
        if trainflag:
            if len(idx1.shape) > 0:
                similarity = torch.mean(
                    torch.sum(fm1[idx1] * fm2[i][idx2], dim=1), dim=0
                )
            else:
...
def rerank(predictions, distances, queries_local_features, database_local_features):
    pred2 = []
    dist2 = []
    for query_index, pred in enumerate(
        tqdm(predictions, desc="SelaVPR Match Re-Ranking")
    ):
        ...
        if torch.cuda.is_available():
            query_local_features = query_local_features.cuda()
            candidates_local_features = candidates_local_features.cuda()
        rerank_index = (
            local_sim(query_local_features, candidates_local_features)
            .cpu()
            .numpy()
            .argsort()[::-1]
        )
        pred2.append(predictions[query_index][rerank_index])
        dist2.append(distances[query_index][rerank_index])
    return np.array(dist2), np.array(pred2)
...
class SelaVPR(TwoStageBaseModelWrapper):
    ...
    def place_recognise(
        ...
        """
        if isinstance(query, torch.utils.data.DataLoader):
            descs = self.compute_query_desc(dataloader=query, pbar=pbar)
            query_desc_global = descs["global_descriptors"]
            query_desc_local = descs["local_descriptors"]
        elif isinstance(query, Dict):
            query_desc_global = query["global_descriptors"].astype(np.float32)
            query_desc_local = query["local_descriptors"].astype(np.float32)
        elif isinstance(query, Image.Image):
            query_desc_local, query_desc_global = self.compute_feature(query)

        if config["eval"]["distance"] == "cosine":
            faiss.normalize_L2(query_desc_global)
        dist, predictions = self.map.search(query_desc_global, k)

        dist, predictions = rerank(
            predictions, dist, query_desc_local, self.map_desc["local_descriptors"]
        )
