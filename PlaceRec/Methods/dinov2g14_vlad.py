    ...
    [
        ...
        transforms.Lambda(
            lambda img: transforms.functional.center_crop(
                img, (make_patchable(img.shape[1]), make_patchable(img.shape[2]))
            )
        ),
    ]
)
...
class AnyLoc(nn.Module):
    def __init__(self):
        ...
        cache_dir = _ex("./cache")
        ext_specifier = f"dinov2_vitg14/" f"l31_value_c32"
        c_centers_file = os.path.join(
            cache_dir, "vocabulary", ext_specifier, "urban", "c_centers.pt"
        )
        assert os.path.isfile(c_centers_file), "Vocabulary not cached!"
        c_centers = torch.load(c_centers_file)
        assert c_centers.shape[0] == 32, "Wrong number of clusters!"

        self.dino = DinoV2ExtractFeatures("dinov2_vitg14", 31, "value", device="cpu")
        self.dino.dino_model.eval()
        self.vlad = VLAD(32, desc_dim=None, cache_dir=os.path.dirname(c_centers_file))
        self.vlad.fit(None)

    def forward(self, x):
        tokens = self.dino(x)
        desc = self.vlad.generate(tokens.cpu().squeeze())
        return desc
...
class DINOv2_AnyLoc(BaseModelWrapper):
    def __init__(self, pretrained: bool = True):
        ...
        super().__init__(model=model, preprocess=preprocess, name="dinov2_anyloc")
