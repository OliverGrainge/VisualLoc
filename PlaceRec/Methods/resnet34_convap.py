...
class ResNet34ConvAP(BaseModelWrapper):
    def __init__(self, pretrained: bool = True):
        if pretrained:
            ...
            model = VPRModel(backbone_arch='resnet34', 
                            layers_to_crop=[],
                            agg_arch='ConvAP',
            ...
            state_dict = torch.load("/home/oliver/Documents/github/VisualLoc/PlaceRec/Training/Distillation/checkpoints/distillation/resnet34convap/resnet34convap_single_resolution_320_420-epoch=00-val_loss=0.00.ckpt")
            new_state_dict = {}
            for key in list(state_dict["state_dict"].keys()):
                print(key)
                new_state_dict[key.replace("model.backbone.model", "backbone.model").replace("model.aggregator", "aggregator")] = state_dict["state_dict"][key]
