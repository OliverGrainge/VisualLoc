class DinoV2ExtractFeatures:
    ...
    """
    def __init__(self, dino_model: _DINO_V2_MODELS, layer: desc_layer, 
                facet: _DINO_FACETS, use_cls=False, 
                norm_descs=True, device: str = "cuda") -> None:
        """
            Parameters:
...
for v in range(10):
    ...
    with torch.no_grad():
        ...
        pil_img = Image.fromarray(np.random.randint(0, 155, size=(320, 320, 3)).astyp(np.uint8))
        ...
        if max(img_pt.shape[-2:]) > max_img_size:
            ...
            print(f"Resized {v} to {img_pt.shape = }")
...
extractor = DinoV2ExtractFeatures(device="cpu")
vlad = VLAD(num_c, desc_dim=None, 
    cache_dir=os.path.dirname(c_centers_file))