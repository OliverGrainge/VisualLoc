...
class GeM(nn.Module):
    ...
    def forward(self, x):
        return F.avg_pool2d(x.clamp(min=self.eps).pow(self.p), (x.size(-2), x.size(-1))).pow(1./self.p).view(x.shape[0], -1)
...
preprocess = transforms.Compose([
    ...
    transforms.Normalize(              # Normalize the image
        mean=[0.485, 0.456, 0.406],    # These are the ImageNet mean and std values
