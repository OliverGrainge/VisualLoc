...
class MultiPlexVPR(BaseTechnique):
    def __init__(self):
        ...
        self.selection_model = ResNet18.load_from_checkpoint(
            SELECTION_MODEL_PATH, output_dim=len(METHODS)
        )
    ...
    def compute_query_desc(
        ...
        if images is not None and dataloader is not None:
            ...
            progress_bar = tqdm(
                total=len(paths),
                desc="Computing MultiPlexVPR Query Descriptors",
                leave=True,
                disable=not pbar,
            )
            selections = self.compute_selections(
                dataloader=dataloader, images=None
            ).numpy()
            select_masks = [
                np.where(np.array(selections) == i) for i in range(len(METHODS))
            ]
            ...
            for i, queries in enumerate(select_queries):
                if len(queries) >= 1:
                    ds = ImageDataset(
                        img_paths=queries, preprocess=self.methods[i].preprocess
                    )
                    dl = DataLoader(ds, shuffle=False, batch_size=BATCHSIZE)
                    progress_bar.update(ds.__len__())
                    desc = self.methods[i].compute_query_desc(
                        dataloader=dl, pbar=False
                    )["query_descriptors"]
