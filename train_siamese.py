...
class SiameseResnet18(pl.LightningModule):
    def __init__(self, output_dim):
        ...
        self.subnetwork = resnet18(weights=ResNet18_Weights.DEFAULT)
        self.subnetwork.fc = nn.Linear(512, 512)
    ...
    def forward(self, x):
        q_img, ref_img = x
        q_output = self.subnetwork(q_img)
        r_output = self.subnetwork(ref_img)
...
if __name__ == "__main__":
    ...
    logger = TensorBoardLogger("tb_logs", name="siamese_model")

    checkpoint_callback = ModelCheckpoint(
        monitor="val_loss",
        dirpath="SelectionNetworkCheckpoints",
        filename=WEIGHTS_NAME + "_SiameseResnet18-{epoch:02d}-{val_loss:.2f}",
        save_top_k=1,
        mode="min",
    )
