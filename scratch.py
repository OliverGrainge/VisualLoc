...
class cct384_netvlad(nn.Module):
    def __init__(self):
        super().__init__()
        self.backbone = cct_8_7x2_384(
            pretrained=False, progress=True, aggregation="netvlad"
        )
    ...
    def __init__(self, pretrained: bool = True):
        ...
        if pretrained:
            model.load_state_dict(
                torch.load(
                    os.path.join(filepath, "weights", "msls_cct384tr8fz1_netvlad.pth")
                )
            )
...
sd = torch.load(
    "/Users/olivergrainge/Documents/github/VisualLoc/PlaceRec/Methods/weights/msls_cct384tr8fz1_netvlad.pth",
    map_location="cpu",
)["model_state_dict"]
...
if list(loaded_sd.keys())[0].startswith("module"):
    loaded_sd = OrderedDict(
        {k.replace("module.", ""): v for (k, v) in loaded_sd.items()}
    )
