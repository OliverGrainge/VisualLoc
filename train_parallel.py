...
class ParallelResnet18(pl.LightningModule):
    def __init__(self, output_dim):
        ...
        self.model1 = resnet18(weights=ResNet18_Weights.DEFAULT)
        self.model2 = resnet18(weights=ResNet18_Weights.DEFAULT)

        self.model1.fc = nn.Linear(512, 512)
        self.model2.fc = nn.Linear(512, 512)
    ...
    def forward(self, x):
        q_img, ref_img = x
        q_output = self.model1(q_img)
        r_output = self.model2(q_img)

        # Concatenate outputs
        combined = torch.cat((q_output, r_output), dim=1)
...
if __name__ == "__main__":
    ...
    logger = TensorBoardLogger("tb_logs", name="parallel_model")

    checkpoint_callback = ModelCheckpoint(
        monitor="val_loss",
        dirpath="SelectionNetworkCheckpoints",
        filename=WEIGHTS_NAME + "_ParallelResnet18-{epoch:02d}-{val_loss:.2f}",
        save_top_k=1,
        mode="min",
    )
